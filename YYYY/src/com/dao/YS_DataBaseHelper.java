...
public class YS_DataBaseHelper {
	...
	public YS_DataBaseHelper(Context context, DataBase dataBase) {
	...
	public Cursor read_budget() {
		...
		Cursor cursor;
		cursor = db.rawQuery("select budget from tablebudget where month = '"
				+ currentString + "'", null);
	 ...
	 * 更新总预算表,被每次记一笔时调用
	 */
	private boolean deltotal(float consume, SQLiteDatabase db) {
	...
	public boolean update(float consume, int kind) {
		SQLiteDatabase db = dataBase.getWritableDatabase();
		db.execSQL("update tablebudget set remain = remain -" + consume
	 ...
	 * @return
	 */
	public boolean updatein(float in, SQLiteDatabase db) {
		db = dataBase.getWritableDatabase();
